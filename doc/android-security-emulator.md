...
Android announcing a VPN network, does not establish readiness. During the first
cold-start `CONNECTING` transition, traffic may still use the physical route
before TUN descriptor delivery to tun2socks and `CONNECTED`. The readiness test
observes the new session's state transition, and the
...
network. Do not disable the host VPN. Check the operating system's active DNS
configuration (`scutil --dns` on macOS) and choose a reachable resolver for the
current environment rather than copying a private address from another computer.
